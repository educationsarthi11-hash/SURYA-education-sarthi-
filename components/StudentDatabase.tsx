        ...
        return (
            <div className="animate-pop-in">
                ...
                <div className="bg-neutral-50 p-6 rounded-lg border">
                    ...
                    <p className="font-mono text-sm text-neutral-500">{selectedStudent.id}</p>

                    {/* Tabs */}
                    <div className="flex border-b mt-6 mb-4">
                        <button
                            onClick={() => setActiveProfileTab('overview')}
                            className={`px-4 py-2 text-sm font-medium ${activeProfileTab === 'overview' ? 'text-primary border-b-2 border-primary' : 'text-neutral-500 hover:text-neutral-700'}`}
                        >
                            Overview
                        </button>
                        <button
                            onClick={() => setActiveProfileTab('documents')}
                            className={`px-4 py-2 text-sm font-medium flex items-center gap-2 ${activeProfileTab === 'documents' ? 'text-primary border-b-2 border-primary' : 'text-neutral-500 hover:text-neutral-700'}`}
                        >
                            <ArchiveBoxIcon className="h-4 w-4"/> Documents & Certificates
                        </button>
                    </div>
                    ...
                    {activeProfileTab === 'overview' ? (
                        <>
                            <div className="mt-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2 text-sm">
                                    ...
                                    <p className="flex items-center gap-2">
                                        <strong>Mobile:</strong> {selectedStudent.mobileNumber}
                                        <span className="inline-flex gap-2 no-print">
                                            <a href={`tel:${selectedStudent.mobileNumber}`} title="Call" className="text-neutral-500 hover:text-primary"><PhoneIcon className="h-4 w-4"/></a>
                                            <a href={`sms:${selectedStudent.mobileNumber}`} title="Send SMS" className="text-neutral-500 hover:text-primary"><EnvelopeIcon className="h-4 w-4"/></a>
                                            <a href={`https://wa.me/${formatPhoneNumberForWhatsApp(selectedStudent.mobileNumber)}`} target="_blank" rel="noopener noreferrer" title="Send WhatsApp" className="text-neutral-500 hover:text-green-500"><WhatsAppIcon className="h-4 w-4"/></a>
                                        </span>
                                    </p>
                            ...
                            <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <div className="bg-white p-4 rounded-md shadow-sm border">
                                        <p className="text-sm text-neutral-600 mb-2">Recent Performance:</p>
                                        {studentRecords.length > 0 ? (
                                            <ul className="space-y-2">
                                                {studentRecords.slice(0, 3).map(rec => (
                                                    <li key={rec.recordId} className="flex justify-between text-sm border-b pb-1 last:border-0">
                                                        <span>{rec.examName}</span>
                                                        <span className="font-bold">{rec.overallPercentage}%</span>
                                                    </li>
                                                ))}
                                            </ul>
                                        ) : <p className="text-sm text-neutral-400">No academic records found.</p>}
                                    </div>
                                </div>
                                <div>
                                    <h4 className="font-bold text-lg mb-2">Fee Status</h4>
                                    <div className="bg-white p-4 rounded-md shadow-sm border">
                                        <div className="flex justify-between items-center mb-2">
                                            <span className="text-sm text-neutral-600">Total Due:</span>
                                            <span className="text-xl font-bold text-neutral-900">₹{feeSummary.totalDue.toLocaleString()}</span>
                                        </div>
                                        <div className="flex justify-between items-center">
                                            <span className="text-sm text-neutral-600">Status:</span>
                                            <span className={`px-2 py-1 rounded-full text-xs font-bold ${feeSummary.status === 'Paid' ? 'bg-green-100 text-green-800' : feeSummary.status === 'Overdue' ? 'bg-red-100 text-red-800' : 'bg-amber-100 text-amber-800'}`}>
                                                {feeSummary.status}
                                            </span>
                                        </div>
                                    </div>
                            ...
                            {/* AI Report Section */}
                            {isGeneratingReport && <div className="mt-6 p-4 border rounded-lg bg-white text-center"><Loader message="AI is analyzing performance and generating report..."/></div>}
                            {aiReportHtml && (
                                <div className="mt-6 p-6 bg-white rounded-lg border shadow-sm animate-pop-in printable-content" ref={reportRef}>
                                    <div className="flex justify-between items-center mb-4 border-b pb-2">
                                        <h3 className="text-xl font-bold text-primary flex items-center gap-2">
                                            <SparklesIcon className="h-6 w-6"/> AI Student Progress Report
                                        </h3>
                                        <button onClick={handlePrintReport} className="p-2 bg-neutral-100 rounded-full hover:bg-neutral-200 no-print"><PrinterIcon className="h-5 w-5 text-neutral-600"/></button>
                                    </div>
                                    <div className="prose prose-sm max-w-none text-neutral-700" dangerouslySetInnerHTML={{ __html: aiReportHtml }} />
                                </div>
                            )}
                        </>
                    ) : (
                        ...
                        <div className="mt-4 animate-pop-in">
                            <div className="flex justify-between items-center mb-4">
                                ...
                                <div className="relative">
                                    ...
                                    <button 
                                        ...
                                        className="flex items-center gap-2 px-3 py-1.5 bg-white border border-neutral-300 rounded-md text-sm hover:bg-neutral-50 shadow-sm"
                                    >
                                        <UploadIcon className="h-4 w-4 text-neutral-500" /> Upload Record
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {documents.map((doc) => (
                                    <div key={doc.id} className="group bg-white p-4 rounded-lg border border-neutral-200 hover:shadow-md transition-all relative">
                                        <div className="flex items-start gap-3">
                                            <div className="bg-blue-50 p-2 rounded-lg text-blue-600">
                                                <FolderIcon className="h-6 w-6" />
                                            </div>
                                            <div className="flex-1 min-w-0">
                                                <h5 className="font-semibold text-sm text-neutral-800 truncate" title={doc.title}>{doc.title}</h5>
                                                <p className="text-xs text-neutral-500 mt-0.5">{doc.type} • {doc.date}</p>
                                            </div>
                                        </div>
                                        <div className="mt-4 flex gap-2 border-t pt-3">
                                            <button className="flex-1 text-xs font-medium text-neutral-600 hover:text-primary flex items-center justify-center gap-1 py-1 hover:bg-neutral-50 rounded">
                                ...
                                <button 
                                    onClick={() => fileInputRef.current?.click()}
                                    className="border-2 border-dashed border-neutral-200 rounded-lg p-4 flex flex-col items-center justify-center text-neutral-400 hover:border-primary/50 hover:text-primary hover:bg-primary/5 transition-colors"
                                >
