...
import { CameraIcon, XIcon, SparklesIcon, PhotoIcon, ArrowPathIcon, CheckCircleIcon } from './icons/AllIcons';
...
const UnifiedScanner: React.FC<UnifiedScannerProps> = ({ 
    ...
    const startCamera = async () => {
        try {
            ...
            toast.error("कैमरा एक्सेस नहीं मिला। (Camera access denied)");
        }
    };
    ...
    const handleAnalyze = async () => {
        ...
        try {
            ...
            toast.success("AI स्कैन सफल! (Scan Successful)");
            handleReset();
            onClose();
        } catch (e) {
            toast.error("AI विश्लेषण विफल रहा। (Analysis Failed)");
        } finally {
            setLoading(false);
        }
    };
                    ...
                    {!capturedFile ? (
                        <>
                            ...
                            <div className="absolute inset-0 pointer-events-none">
                                ...
                                {/* Center Target */}
                                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-0.5 bg-primary/20"></div>
                                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-full w-0.5 bg-primary/20"></div>
                            </div>
                            ...
                            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center pointer-events-none">
                                <p className="text-white font-hindi text-xl font-bold bg-black/40 px-6 py-2 rounded-full border border-white/20 backdrop-blur-md">
                                    डॉक्यूमेंट को कोनों में सेट करें
                                </p>
                            </div>
                        </>
                    ) : (
                        <div className="w-full h-full flex flex-col p-6 animate-pop-in">
                            {loading ? (
                                ...
                                <>
                                    <div className="flex-1 overflow-hidden rounded-[2rem] border-4 border-slate-800 shadow-2xl bg-slate-800">
