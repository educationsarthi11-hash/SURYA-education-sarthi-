            ...
            <div className="flex-1 space-y-4 overflow-y-auto pr-2 custom-scrollbar">
                {affiliations.map((aff, i) => (
                    <div key={i} className="p-5 bg-slate-50 border border-slate-100 rounded-[2rem] hover:bg-white hover:shadow-xl transition-all group cursor-pointer relative overflow-hidden">
                        <div className={`absolute top-0 left-0 w-1.5 h-full bg-${aff.color === 'orange' ? 'orange-500' : aff.color === 'blue' ? 'blue-500' : 'green-500'}`}></div>
                        <div className="flex justify-between items-center relative z-10">
                            ...
                            <span className={`text-[9px] font-black px-2.5 py-1 rounded-full uppercase tracking-tighter ${
                                aff.color === 'orange' ? 'bg-orange-100 text-orange-700' : 
                                aff.color === 'blue' ? 'bg-blue-100 text-blue-700' : 
